...
const tools = [
    {
        name: "chat",
        description: "Send a message to an AI model via LightningProx. Pay per request with a Lightning spend token. Supports 19 models from Anthropic, OpenAI, Together.ai, Mistral, and Google.",
        inputSchema: {
            type: "object",
            properties: {
                model: {
                    type: "string",
                    description: "Model ID (e.g. claude-opus-4-5-20251101, gpt-4-turbo, gemini-2.5-pro, mistral-large-latest, deepseek-ai/DeepSeek-V3)",
                },
                message: {
                    type: "string",
    ...
    {
        name: "list_models",
        description: "List all AI models available through LightningProx. Returns model IDs, names, providers, and pricing. 19 models across Anthropic, OpenAI, Together.ai, Mistral, and Google.",
        inputSchema: {
            type: "object",
            properties: {},
            required: [],
    ...
    {
        name: "get_balance",
        description: "Check the remaining balance on a LightningProx spend token. Returns balance in sats. Uses LIGHTNINGPROX_SPEND_TOKEN env var if no token provided.",
        inputSchema: {
            type: "object",
            properties: {
                spend_token: {
    ...
    },
    {
        ...
        description: "Generate a Bitcoin Lightning invoice to top up a LightningProx spend token. Returns a BOLT11 payment request and charge ID. Pay the invoice with any Lightning wallet.",
        inputSchema: {
            type: "object",
            properties: {
                amount_sats: {
    ...
    {
        name: "check_payment",
        description: "Check if a Lightning invoice has been paid and retrieve the spend token. Poll this after generate_invoice until the payment is confirmed.",
        inputSchema: {
            type: "object",
            properties: {
                charge_id: {
...
// API HELPERS
// ============================================================================
async function chat(model, message, spendToken, maxTokens = 1024) {
...
async function getBalance(spendToken) {
    const res = await fetch(`${LIGHTNINGPROX_URL}/v1/balance`, {
        headers: { "X-Spend-Token": spendToken },
    });
    if (!res.ok) {
        const err = await res.json().catch(() => ({}));
        const e = new Error(err.error || `Balance check failed: ${res.status}`);
        e.status = res.status;
        throw e;
    }
    return res.json();
...
async function checkPayment(chargeId) {
...
// MCP SERVER
// ============================================================================
const server = new index_js_1.Server({ name: "lightningprox", version: "1.2.0" }, { capabilities: { tools: {} } });
...
server.setRequestHandler(types_js_1.CallToolRequestSchema, async (request) => {
    ...
    try {
        switch (name) {
            case "chat": {
                const { model, message, spend_token, max_tokens } = args;
